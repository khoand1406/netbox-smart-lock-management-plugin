# Testing Guide for smart_lock_management_plugin

This guide explains how to write and run tests for smart_lock_management_plugin.

## Test Structure

Tests are organized in the `smart_lock_management_plugin/tests/` directory:
...
```python
from smart_lock_management_plugin.testing import PluginTestCase
...
```python
from smart_lock_management_plugin.testing import PluginModelTestCase
...
```python
from smart_lock_management_plugin.testing import PluginAPITestCase

class MyAPITestCase(PluginAPITestCase):
    def setUp(self):
        super().setUp()
        self.list_url_name = 'plugins-api:smart_lock_management_plugin-api:mymodel-list'
        self.detail_url_name = 'plugins-api:smart_lock_management_plugin-api:mymodel-detail'
...
```python
from smart_lock_management_plugin.testing import PluginViewTestCase

class MyViewTestCase(PluginViewTestCase):
    def setUp(self):
        super().setUp()
        self.base_url = 'plugins:smart_lock_management_plugin:mymodel'
...
```python
from smart_lock_management_plugin.testing import PluginGraphQLTestCase
...
The `smart_lock_management_plugin.testing.utils` module provides helpful utilities:

```python
from smart_lock_management_plugin.testing.utils import (
...
user = create_test_user(permissions=['smart_lock_management_plugin.view_smartlockmanagementplugin'])
...
python manage.py test smart_lock_management_plugin.tests

# Run specific test file
python manage.py test smart_lock_management_plugin.tests.test_models

# Run specific test class
python manage.py test smart_lock_management_plugin.tests.test_models.Smart_Lock_Management_PluginTestCase

# Run specific test method
python manage.py test smart_lock_management_plugin.tests.test_models.Smart_Lock_Management_PluginTestCase.test_create

# Run with verbose output
python manage.py test smart_lock_management_plugin.tests -v 2

# Run in parallel (faster)
python manage.py test smart_lock_management_plugin.tests --parallel

# Keep database between runs (faster during development)
python manage.py test smart_lock_management_plugin.tests --keepdb
```
...
# Run tests
docker-compose exec netbox python manage.py test smart_lock_management_plugin.tests

# Run specific tests
docker-compose exec netbox python manage.py test smart_lock_management_plugin.tests.test_api
...
def test_form_validation(self):
    ...
    url = reverse('smart_lock_management_plugin:mymodel_add')
...
**"No module named 'smart_lock_management_plugin'"**
